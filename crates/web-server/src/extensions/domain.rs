	...
	async fn from_request_parts(
		...
		let Extension(pool): Extension<DbPool> = Extension::from_request_parts(parts, state)
			...
			.map_err(|_| {
				return Err((
					...
					GenericMessage::new("Failed to get database extension."),
				));
			})?;
		...
		let host = parts
			...
			.ok_or_else(|| {
				return Err((
					StatusCode::INTERNAL_SERVER_ERROR,
					GenericMessage::new("Invalid host header."),
				));
			})?
			.to_str()
			.map_err(|_| {
				return Err((
					StatusCode::INTERNAL_SERVER_ERROR,
					GenericMessage::new("Failed to parse host header."),
				));
			})?;
		...
		let conn = &mut pool.get().map_err(|_| {
			return Err((
				StatusCode::INTERNAL_SERVER_ERROR,
				GenericMessage::new("Failed to get connection."),
			));
		})?;
		...
		let domain = Domain::get_by_domain(host.to_string(), conn).map_err(|_| {
			return Err((
				...
				GenericMessage::new("Failed to find requested host."),
			));
		})?;
