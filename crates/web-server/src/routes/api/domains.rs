...
use crate::{
	...
	util::{is_admin, is_url, strip_protocol},
};
...
async fn create_domain(
	...
	if !is_url(&payload.domain) {
		return Err((
			...
			GenericMessage::new("Provided domain is not a valid URL."),
		));
	}
	...
	if let Ok(_) = Domain::get_by_domain(stripped_domain.clone(), conn) {
		return Err((
			StatusCode::CONFLICT,
			GenericMessage::new("Domain already exists."),
		));
	}
	...
	match new_domain.insert(conn) {
		...
		Err(_) => Err((
			StatusCode::INTERNAL_SERVER_ERROR,
			GenericMessage::new("Failed to create domain."),
		)),
	}
}
...
async fn get_domains(
	...
	let items =
		Domain::get_paginated(pagination.page, pagination.per_page, conn).map_err(|_| {
			(
				StatusCode::INTERNAL_SERVER_ERROR,
				GenericMessage::new("Internal server error."),
			)
		})?;
	let total_count = Domain::get_total_count(conn).map_err(|_| {
		(
			StatusCode::INTERNAL_SERVER_ERROR,
			GenericMessage::new("Internal server error."),
		)
	...
	Ok((
		StatusCode::OK,
		Json(PaginatedResponse::<Domain> { items, total_count }),
	))
}
