...
use crate::{common::{APIResponse, GenericMessage}, extensions::auth::AuthedUser, types::{PaginatedResponse, PaginationQuery}, util::{is_admin, is_url, strip_protocol}};
...
async fn get_domains(
	...
	let items = Domain::get_paginated(pagination.page, pagination.per_page, conn).map_err(|_| (StatusCode::INTERNAL_SERVER_ERROR, GenericMessage::new("Internal server error.")))?;
	let total_count = Domain::get_total_count(conn).map_err(|_| (StatusCode::INTERNAL_SERVER_ERROR, GenericMessage::new("Internal server error.")))?;

	Ok((StatusCode::OK, Json(PaginatedResponse::<Domain> {
		items,
		total_count,
	})))
}
